...
[ステートマシンのページ](../state-machine/) の前提は「どこもブロックしない」だった。ソケットは `O_NONBLOCK` にすれば `EAGAIN` を返すので、`NGX_AGAIN` に翻訳して帰ればいい。
...
2. **3 つとも、呼び出し側から見た形を「`NGX_AGAIN` を返して、後でイベントとして完了が来る」に揃える。** [ステートマシンのページ](../state-machine/) の中断・再開の形そのままになる。
...
- **`r->main->blocked++`**: [ステートマシンのページ](../state-machine/) で見た `ngx_http_close_request()` が `if (r->count || r->blocked) return;` で参照する。**AIO が走っている間はリクエストを解放してはいけない。** バッファに書き込み中のカーネルが、解放済みメモリを踏む。
...
正常に完了した場合は `r->write_event_handler(r)` を呼ぶ。[ステートマシンのページ](../state-machine/) のとおり、これは「書けるようになった」ときと同じ経路だ。**AIO の完了が、いつもの再開に翻訳されている。**
...
さらに言えば、その「形」は既に [ステートマシンのページ](../state-machine/) で確立していた `NGX_AGAIN` + イベント再開の形だ。**新しい非同期の仕組みを、既存の非同期の語彙に翻訳している。**
...
結果として、**リクエスト処理のコードにロックが 1 つも増えていない**。[ステートマシンのページ](../state-machine/) の「スレッドを足すときは、完了通知を必ず既存のループに戻す」が、ここで具体的に効いている。
...
[ステートマシンのページ](../state-machine/) で見た `ngx_http_close_request()` は `if (r->count || r->blocked) return;` と書いている。2 つとも「解放するな」を意味するのに、なぜ分かれているか。

`count` は **「このリクエストを気にしている論理的な参照の数」**で、[サブリクエスト](../subrequest-postpone/) や [upstream](../upstream-event-pipe/) が増やす。減らせば、いずれ 0 になって解放される。
...
- `NGX_AGAIN` と中断・再開の基本形は [ステートマシンのページ](../state-machine/)。
- `ngx_notify()` の実装と `eventfd` の使い方は [イベントメソッドのページ](../event-methods/)。
- `r->main->blocked` を参照する `ngx_http_close_request()` は [ステートマシンのページ](../state-machine/)。
