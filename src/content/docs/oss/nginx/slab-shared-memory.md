...
これが呼ばれるのは `ngx_init_cycle()` の中 ([起動と cycle のページ](../boot-cycle/))、つまり [master/worker のページ](../master-worker/) で見たとおり **fork より前**だ。だから全ワーカーがこのマッピングを継承する。
...
**「まとめて掃除する」ではなく「使うたびに少しずつ掃除する」**を選んでいる。GC の世界でいうインクリメンタル回収と同じ発想で、レイテンシの尖りを避けるための選択だ。イベントループを 1 スレッドで回している以上、どこかで長く止まるのが一番まずい ([ワーカーの 1 周のページ](../state-machine/))。
...
- このゾーンを一番大きく使うのは [キャッシュのページ](../file-cache/) の索引の赤黒木。
