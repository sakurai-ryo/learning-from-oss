...
上のコードで両方に付いている `flags | BTR_NO_LOCKING_FLAG` は、**この時点でクラスタード行のロック評価をもう一度行わないための印**だ。ロックは前段の locking read (または delete-mark 段階) ですでに確定しており、書き込みの直前でロックを取り直す関数 (`lock_clust_rec_modify_check_and_lock`) はこのフラグを見て素通りする。ロック取得の視点からこの経路をもう一段掘り下げたものが [UPDATE / DELETE のロック取得点](./update-and-delete-locking/) にある。
