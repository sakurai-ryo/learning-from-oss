...
[SQLite のページ](../sqlite-state/) の排他トランザクションは、1 回の読み書きの整合性しか守らない。「状態を読み、判断し、conmon を起動し、書き戻す」という一連の操作の途中で、別のプロセスが同じコンテナを止めに来ないことを保証するには、コンテナ単位のミューテックスが要る。それも **プロセスをまたいで** 効くものが。プロセス内ロックで済まなくなった経緯は [デーモンがあると何ができて、無いと何が難しいか](../daemon-or-not/) を参照。
