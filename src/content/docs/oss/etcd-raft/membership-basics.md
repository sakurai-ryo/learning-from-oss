...
構成の遷移そのものも、ログに載るエントリとして進む。

```mermaid
stateDiagram-v2
    direction LR
    Cold: C_old = {1,2,3}
    Joint: C_old,new = {1,2,3} かつ {1,2,4,5,6}
    Cnew: C_new = {1,2,4,5,6}

    Cold --> Joint: EntryConfChangeV2 (joint に入る) を<br/>ログに書いた時点で切り替わる
    Joint --> Cnew: 空の EntryConfChangeV2 (joint を抜ける) を<br/>ログに書いた時点で切り替わる
...
次のページで、この 8 つの規則が実際に噛み合うところを、3 台のクラスタの時間軸の上で通しで見る。
